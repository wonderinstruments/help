...
The first method is a specially-named method called `__init__()`.
...
that we call the `__init__()` method in the `PartyAnimal` so that whatever setup
that `PartyAnimal` needs is done in addition to the setup needed for the `CricketFan`
extensions.
